    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <div className="mx-auto grid max-w-md gap-8 px-6 py-20">
        ...
        <form
          ...
          className="animate-in animate-in-delay-1 space-y-4 rounded-2xl border border-border bg-surface p-6 transition-all duration-300 hover:glow-sm"
        >
          {mode === "signup" && (
            <div className="group relative">
              <User className="pointer-events-none absolute left-4 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" />
              <input
                className="w-full rounded-xl border border-border bg-background px-11 py-3.5 text-sm outline-none transition-all duration-300 focus:border-brand/50 focus:shadow-lg focus:shadow-brand/5"
                placeholder="Prénom"
              />
            </div>
          )}
          ...
          <button className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-brand py-3.5 text-sm font-black uppercase tracking-wider text-brand-foreground transition-all duration-300 hover:glow">
