      ...
      <div className="mx-auto max-w-7xl px-4 pb-24 pt-8 md:px-6">
        ...
        <div className="mt-10 grid gap-8 lg:grid-cols-[1fr_380px]">
          <div className="space-y-12">
                ...
                {GYMS.map((g) => {
                  ...
                  return (
                    <button
                      key={g.id}
                      onClick={() => setGym(g)}
                      className={`group rounded-xl border bg-surface p-4 text-left transition-all duration-300 ${
                        sel
            ...
            {reco && macros && (
              <Section
                ...
                completed={!!reco}
              >
                <div className="overflow-hidden rounded-2xl border border-border bg-surface transition-all duration-300 hover:glow-sm">
                          ...
                          {DIETS.map((d) => {
                            ...
                            return (
                              <button
                                ...
                                className={`rounded-full px-3 py-1 text-[11px] font-semibold transition-all duration-300 ${
                                  on
                                    ? "bg-brand text-brand-foreground shadow-lg shadow-brand/20"
                ...
                {slots.map((s) => {
                  ...
                  return (
                    <button
                      key={s}
                      onClick={() => setPickup(s)}
                      className={`rounded-lg border px-2 py-2.5 text-sm font-bold transition-all duration-300 ${
                        sel
          ...
          <aside className="lg:sticky lg:top-24 lg:h-fit">
            <div className="rounded-2xl border border-border bg-surface p-6 transition-all duration-300 hover:glow-sm">
              ...
              <button
                onClick={addToCart}
                disabled={!ready}
                className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-brand py-4 text-sm font-black uppercase tracking-wider text-brand-foreground transition-all duration-300 hover:glow disabled:cursor-not-allowed disabled:opacity-40"
              >
      ...
      {steps.map((s, i) => {
        ...
        return (
          <div key={s} className="flex flex-1 items-center gap-1">
            <div
              className={`flex items-center gap-2 rounded-full px-3 py-1.5 text-[10px] font-bold uppercase tracking-widest transition-all duration-500 ${
                done
                  ? "bg-brand text-brand-foreground"
                  : active
            ...
            {i < steps.length - 1 && (
              <div
                className={`ml-1 h-px flex-1 transition-all duration-500 ${done ? "bg-brand/50" : "bg-border"}`}
              />
            )}
          </div>
        );
