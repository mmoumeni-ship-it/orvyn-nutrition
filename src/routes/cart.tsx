    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <div className="mx-auto max-w-7xl px-6 py-12">
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[1fr_400px]">
          <div className="space-y-4">
            {cart.length === 0 ? (
              <div className="animate-in rounded-2xl border border-dashed border-border bg-surface/40 p-16 text-center">
                ...
                <Link
                  to="/menu"
                  className="group mt-6 inline-flex items-center gap-2 rounded-full bg-brand px-6 py-3 text-xs font-black uppercase tracking-widest text-brand-foreground transition-all duration-300 hover:glow"
                >
                  Explorer le menu{" "}
              ...
              cart.map((l, i) => (
                <div
                  key={l.id}
                  className="animate-in flex items-start justify-between gap-4 rounded-2xl border border-border bg-surface p-5 transition-all duration-300 hover:border-brand/30 hover:glow-sm"
                ...
                >
                  ...
                  <div className="flex flex-col items-end gap-3">
                    ...
                    <button
                      onClick={() => remove(l.id)}
                      className="rounded-lg p-2 text-muted-foreground transition-all duration-300 hover:bg-destructive/10 hover:text-destructive"
                    >
            ...
            {cart.length > 0 && (
              <div className="animate-in rounded-2xl border border-border bg-surface p-5 transition-all duration-300 hover:glow-sm">
          ...
          <aside>
            <div className="animate-in rounded-2xl border border-border bg-surface p-6 transition-all duration-300 hover:glow-sm">
              ...
              <div className="mt-2 grid grid-cols-2 gap-2">
                <button
                  ...
                  className={`flex items-center gap-2 rounded-lg border p-3 text-xs font-bold transition-all duration-300 ${
                    mode === "card"
                ...
                <button
                  ...
                  className={`flex items-center gap-2 rounded-lg border p-3 text-xs font-bold transition-all duration-300 disabled:opacity-40 ${
                    mode === "credits"
                      ? "border-brand bg-brand/5 shadow-lg shadow-brand/10"
                      : "border-border bg-background hover:border-brand/40"
                  }`}
              ...
              <button
                ...
                className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-brand py-4 text-sm font-black uppercase tracking-wider text-brand-foreground transition-all duration-300 hover:glow disabled:opacity-40"
              >
